defmodule AttestoPhoenix.TestHTTPServer do
  @moduledoc false
  ...
  def expect_once(server, method, path, responder) when is_function(responder, 1) do
    put_expectation(server, method, path, :once, responder)
  end

  def expect(server, method, path, responder) when is_function(responder, 1) do
    put_expectation(server, method, path, :many, responder)
  end
  ...
  def verify!(server) do
    if Process.alive?(server.pid) do
      case GenServer.call(server.pid, :verify) do
        :ok -> :ok
        {:error, failures} -> raise ExUnit.AssertionError, message: render_failures(failures)
      end
    end
  end

  def stop(server) do
    if Process.alive?(server.pid), do: GenServer.stop(server.pid, :normal)
  catch
    :exit, _reason -> :ok
  end
  ...
  def handle_call(:verify, _from, state) do
    failures = state.handler_errors ++ pending_expectations(state.expectations) ++ state.unexpected_requests
  ...
  def terminate(_reason, state) do
    if state.server_pid && Process.alive?(state.server_pid), do: Process.exit(state.server_pid, :shutdown)
    :ok
  end

  defp put_expectation(server, method, path, mode, responder) do
    expectation = %{
      route: {normalize_method(method), path},
      mode: mode,
      responder: responder,
      calls: 0
    }
  ...
  defp pop_responder(expectations, request) do
    case Enum.find_index(expectations, &available_match?(&1, request)) do
      ...
      index ->
        ...
        {:ok, expectation.responder, List.replace_at(expectations, index, updated)}
    end
  end

  defp available_match?(%{mode: :once, calls: calls}, _request) when calls >= 1, do: false

  defp available_match?(%{route: {expected_method, expected_path}}, %{method: method, path: path}) do
    expected_method == method && expected_path == path
  end

  defp pending_expectations(expectations) do
  ...
  defp render_failure(%{route: {method, path}, mode: mode, calls: calls}) do
    "Expected #{mode} request for #{method} #{path}, received #{calls}."
  end
  ...
  defmodule Plug do
    ...
    def call(conn, owner) do
      case GenServer.call(owner, {:request, conn.method, conn.request_path}) do
        {:ok, responder} -> run_responder(conn, owner, responder)
        {:error, :unexpected_request} -> Conn.resp(conn, 500, "Unexpected request")
      end
    end
