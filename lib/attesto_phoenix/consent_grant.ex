defmodule AttestoPhoenix.ConsentGrant do
  ...
  owner saw — the same client, redirect URI, scope set, and PKCE challenge — and
  nothing else. This module builds that binding from an
  ...
  The binding is the tuple `(subject, client_id, redirect_uri, scope, code_challenge)`:
  ...
  normalized away; `code_challenge` is `nil` when the request carries no PKCE
  challenge.
  """
  @type binding :: %{
          subject: String.t(),
  ...
  request whose `client_id`, `redirect_uri`, `scope`, and `code_challenge` the
  user saw); `subject` is the authenticated resource owner's OIDC `sub`. The
  returned map feeds `binding_hash/1` and the store's `mint/2` / `consume/2`.
  """
  ...
  order is not significant (RFC 6749 §3.3); a missing `code_challenge` hashes as
  the empty string. Identical on the mint and consume sides for the same request.
  """
