defmodule AttestoPhoenix.Controller.OpenIDConfigurationController do
  ...
  same set; enabled optional grants are added only when that setting is nil,
  while an explicit list must contain their exact URNs);
  `token_endpoint_auth_methods_supported`
  lists the client-authentication methods it accepts. Its default is
  `client_secret_basic`, `client_secret_post`, `private_key_jwt`, and `none`
  for PKCE-using public clients; an explicit config list may add mTLS or
  wallet-attestation methods. The OpenID Connect request-parameter flags
